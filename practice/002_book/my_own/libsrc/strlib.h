 ...
 * File: strlib.h
 * --------------
 * The strlib.h file defines the interface for s simple
 ...
 * abstraction for working with strings, t is not appropriate
 * for all applications. In this interface, the function that
 * return string values (such as Concat and SubString) do so
 * by allocating new memory. Over time, a program that uses
 * this package will consume increasinf amounts of memory
 * and eventually exhaust the available supply. If you are
 * writing a program that runs for a short time and stops,
 * the fact that the package consume memory is not a problem.
 ...
 * to end. For example, Cncat("ABC", "DE") returns the string
 * "ABCDE".
 */
 ...
 * --------------------------
 * This function returns the character at postion i in the
 ...
 * 3. I p2 < p1, SubString returns the empty string.
 */
 ...
 * for character, which is usually ASCII.
 */
 ...
 * ----------------------------------------
 * Beginnig at position start in the string text, this
 ...
 * string of didits. For example, IntegerToString(123)
 * return "123" as string.
 */
 ...
 * ------------------------------
 * This function converts string of digits into a integer.
 ...
 * RealToString(23.45) return "23.45". The conversion is 
 ...
 * legal floating-point number or if it contains extaneous
